...
MESSAGES = {
    ...
    'you_are_out': "Вы выбыли из игры ({reason}). Возвращаемся в главное меню.", # Новое сообщение
...
def process_night_actions(room_id):
    ...
    # Отправляем общее сообщение всем живым
    ...
    # --- Обработка выбывшего игрока после ночи ---
    if killed_player_id:
        if killed_player_id in room['alive_players']: # Убеждаемся, что игрок еще жив перед удалением
            # Сохраняем имя и роль для сообщения, пока игрок в комнате
            ...
            # Отправляем сообщение выбывшему игроку
            try:
                ...
                user_states[killed_player_id] = None # Сбрасываем состояние
            except telebot.apihelper.ApiTelegramException as e:
        ...
        if executed_player_id in room['alive_players']:
            ...
            try:
                bot.send_message(executed_player_id, MESSAGES['you_are_out'].format(reason=f"был казнён голосованием (ваша роль: {executed_player_role})"), 
                                 reply_markup=main_menu_keyboard(), parse_mode='Markdown')
                user_states[executed_player_id] = None # Сбрасываем состояние
            except telebot.apihelper.ApiTelegramException as e:
