...
TOKEN = '8221831967:AAGaVkn259zFu1f-MUjJU-hfcRPZBpGIk94' # Исправленный токен!
...
# Изменена функция get_player_game_keyboard: убрана кнопка с тикающим таймером
def get_player_game_keyboard(room_id, player_id): # Убрал timer_seconds из аргументов
    keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    room = rooms.get(room_id)
    if not room:
    ...
    # Убран тикающий таймер, вместо него просто указываем длительность фазы
...
MESSAGES = {
    ...
    'night_falls': "Город засыпает. Все жители закрывают глаза. ⏳ 1 минута.", # Изменено на 1 минуту
    ...
    'start_voting_day': "Начинается дневное голосование. Обсудите и выберите, кого вы хотите посадить. ⏳ 1 минута.", # Изменено на 1 минуту
...
def assign_roles(player_ids):
    ...
    if 4 <= num_players <= 5:
        ...
        num_mafia = 3
    else: # На случай, если каким-то образом будет другое количество игроков
        logger.error(f"Invalid number of players for role assignment: {num_players}")
        # Можно добавить логику для обработки ошибки или использовать дефолтное распределение
        return {}
    
    # Мирные жители - это все, кто не мафия, шериф, или доктор
...
# Вспомогательная функция для получения оставшегося времени (логика таймера остается на фоне)
...
# Обновленная функция send_message_to_alive_players
def send_message_to_alive_players(room_id, text, keyboard_func=None, parse_mode=None, exclude_player_id=None):
    ...
    # timer_val больше не передается в keyboard_func
    for p_id in list(room['alive_players']):
        if p_id == exclude_player_id: continue
        try:
            keyboard = keyboard_func(room_id, p_id) if keyboard_func else None # timer_val убран
            bot.send_message(p_id, text, reply_markup=keyboard, parse_mode=parse_mode)
            # room['last_sent_message'][p_id] = sent_message.message_id # Теперь не обновляем message_id для каждого сообщения
...
# Функция update_timer_and_check_afk изменена, чтобы не отправлять сообщения с тикающим таймером
def update_timer_and_check_afk(room_id):
    ...
    # Логика отправки сообщений с тикающим таймером удалена
    # Теперь мы просто ждем, пока таймер истечет, и ничего не отправляем в чат каждую секунду.

    if actual_current_time_left > 0 and room_id in rooms:
        room['timer_thread'] = threading.Timer(1, update_timer_and_check_afk, args=[room_id]) # Убрал current_time_left - 1
        ...
        if p_id in room['alive_players']: # Убедимся, что игрок еще жив
            ...
            if room_id in rooms:
                send_message_to_alive_players(room_id, f"Игрок *{player_name}* был удален из игры за бездействие.",
                                              keyboard_func=lambda rid, pid: get_player_game_keyboard(rid, pid), parse_mode='Markdown', exclude_player_id=p_id) # timer_val убран
...
def send_night_message(room_id):
    ...
    current_phase_duration = 60 # Теперь 60 секунд (1 минута)
    ...
    for p_id in list(room['alive_players']):
        ...
        try:
            sent_message = bot.send_message(p_id, message_text, reply_markup=get_player_game_keyboard(room_id, p_id)) # timer_val убран
...
def start_day_voting(room_id):
    ...
    for p_id in list(room['alive_players']):
        try:
            sent_message = bot.send_message(p_id, MESSAGES['start_voting_day'], reply_markup=get_player_game_keyboard(room_id, p_id)) # timer_val убран
            room['last_sent_message'][p_id] = sent_message.message_id
        except telebot.apihelper.ApiTelegramException as e:
...
def process_day_voting(room_id):
    ...
    if vote_counts:
        ...
        if len(potential_targets) == 1:
            ...
            send_message_to_alive_players(room_id, MESSAGES['vote_tie_day'], keyboard_func=lambda rid, pid: get_player_game_keyboard(rid, pid)) # timer_val убран
    ...
    if executed_player_id:
        ...
        send_message_to_alive_players(room_id, MESSAGES['executed_player_day'].format(
            player_name=executed_player_name, role_name=executed_player_role), parse_mode='Markdown', keyboard_func=lambda rid, pid: get_player_game_keyboard(rid, pid)) # timer_val убран
        
    else:
        if not vote_counts or len(potential_targets) > 1:
             send_message_to_alive_players(room_id, MESSAGES['no_execution_day'], keyboard_func=lambda rid, pid: get_player_game_keyboard(rid, pid)) # timer_val убран
...
def handle_in_game_players_button(message):
    ...
    if room_id:
        ...
        keyboard = get_player_game_keyboard(room_id, user_id) # timer_val убран
...
def handle_leave_game_button(message):
    ...
    if current_room_id in rooms and room['alive_players']:
        send_message_to_alive_players(current_room_id, MESSAGES['player_leave_room_notify'].format(player_name=player_name), 
                                      keyboard_func=lambda rid, pid: get_player_game_keyboard(rid, pid), parse_mode='Markdown') # timer_val убран
...
def handle_other_messages(message):
    ...
    if room_id and room_id in rooms:
        room = rooms[room_id]
        player_name = room['players'].get(user_id, "Неизвестный")
        current_timer_left = get_remaining_timer_seconds(room_id) # Все еще используется для логирования, но не для кнопок
                    ...
                    for p_id_mafia in mafia_partners_in_game:
                        try:
                            bot.send_message(p_id_mafia, MESSAGES['chat_message_in_game'].format(sender_name=player_name, text=message.text),
                                            reply_markup=get_player_game_keyboard(room_id, p_id_mafia), parse_mode='Markdown')
                        except telebot.apihelper.ApiTelegramException as e:
