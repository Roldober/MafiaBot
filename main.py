...
def add_bots_to_room(room_id, num_bots_to_add):
    ...
    logger.info(f"Добавлено {num_bots_to_add} ботов в комнату {room_id}. Боты: {added_bots}")
    return added_bots
...
def process_night_actions(room_id):
    ...
    for sheriff_id, target_id in sheriff_checks.items():
        if sheriff_id in room['alive_players'] and target_id in room['alive_players'] and sheriff_id not in BOT_IDS:
