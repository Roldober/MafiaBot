...
def main_menu_keyboard():
    ...
    keyboard.add(btn_create, btn_find, btn_available)
    return keyboard
...
def room_waiting_keyboard(room_id, creator_id, current_user_id):
    ...
    if current_user_id == room['creator']:
        btn_delete = types.KeyboardButton("Удалить комнату")
...
def get_player_game_keyboard(room_id, player_id, timer_seconds=None):
    ...
    btn_leave_game = types.KeyboardButton("↪️Выйти из игры")
...
MESSAGES = {
    ...
    'mafia_win': "Мафия победила! Все мирные жители убиты.",
    'citizens_win': "Мирные жители победили! Вся мафия разоблачена.",
...
def start_day_voting(room_id):
    ...
    for p_id in list(room['alive_players']):
        try:
            ...
            logger.error(f"Неизвестная ошибка при отправке сообщения о голосовании игроку {p_id} в комнате {room_id}: {e}")
            if p_id in room['alive_players']:
                room['alive_players'].discard(p_id)
                if p_id in room['players']:
                    del room['players'][p_id]
...
def check_win_condition(room_id):
    ...
    elif mafia_count == 1 and citizen_count == 1:
        logger.info(f"Mafia wins in room {room_id}: 1v1 Mafia vs Citizen.")
        return True, 'Мафия'
...
def handle_create_room_button(message):
    ...
    try:
        bot.send_message(message.chat.id, MESSAGES['ask_players_count'])
    except Exception as e:
...
def process_max_players_count(message):
    try:
        ...
        if not (2 <= max_players <= 12):
            try:
                bot.send_message(message.chat.id, MESSAGES['invalid_max_players'])
            except Exception as e:
...
def handle_find_room_button(message):
    ...
    try:
        bot.send_message(message.chat.id, MESSAGES['ask_room_id'])
    except Exception as e:
...
@bot.message_handler(func=lambda message: message.text == "↪️Выйти из игры" and user_states.get(message.from_user.id) == 'in_room')
def handle_leave_game_button(message):
    ...
    if current_room_id in rooms:
        ...
        if game_over:
            end_game(current_room_id, winner)
            return
...
def handle_other_messages(message):
    ...
    # Сначала ищем комнату, где игрок находится в игре
    for r_id, r_data in rooms.items():
        if user_id in r_data['players'] and r_data['status'] in ['night', 'day', 'voting']:
            room_id = r_id
            break
            
    if room_id and room_id in rooms: # Дополнительная проверка на существование комнаты
        room = rooms[room_id]
        player_role = room['players_roles'].get(user_id)
        ...
        if room['status'] == 'night':
            ...
            for prefix, role in possible_actions.items():
                if message.text.startswith(prefix) and player_role == role:
                    action_prefix = prefix
                    target_player_name_raw = message.text[len(prefix):].strip()
                    break
            
            # Проверяем "Ждать утра" отдельно
            if message.text == "💤Ждать утра" and player_role == 'Мирный житель':
                ...
                action_made = True
                logger.info(f"User {room['players'].get(user_id, 'Unknown')} ({player_role}) chose 'Ждать утра' in room {room_id}.")
                ...
                if target_id:
                    ...
                    action_made = True
                    logger.info(f"User {room['players'].get(user_id, 'Unknown')} ({player_role}) chose to {action_prefix.strip()} {room['players'].get(target_id, 'Unknown')} in room {room_id}.")
                else:
                    try:
                    ...
                    logger.warning(f"User {room['players'].get(user_id, 'Unknown')} ({player_role}) chose invalid target '{target_player_name_raw}' in room {room_id}.")
            ...
            if not action_made: # If not an action, treat as a chat message
                send_message_to_alive_players(room_id, MESSAGES['message_to_players'].format(sender_name=room['players'].get(user_id, "Неизвестный"), text=message.text),
                                              exclude_player_id=user_id, keyboard_func=lambda rid, pid, timer_val: get_player_game_keyboard(rid, pid, timer_val))
                logger.info(f"User {room['players'].get(user_id, 'Unknown')} sent chat message in room {room_id}: '{message.text}'")
                
        # --- Обработка дневных действий (голосование) ---
        elif room['status'] == 'voting':
            action_made = False
            vote_prefix = "🗳️Голосовать за "
            skip_vote_text = "🔇Пропустить голосование"

            if (message.text.startswith(vote_prefix) or message.text == skip_vote_text):
            ...
            if not action_made: # If not an action, treat as a chat message
                send_message_to_alive_players(room_id, MESSAGES['message_to_players'].format(sender_name=room['players'].get(user_id, "Неизвестный"), text=message.text),
                                              exclude_player_id=user_id, keyboard_func=lambda rid, pid, timer_val: get_player_game_keyboard(rid, pid, timer_val))
                logger.info(f"User {room['players'].get(user_id, 'Unknown')} sent chat message during voting in room {room_id}: '{message.text}'")
        
        else: # If status is 'day' but not voting (just chat messages)
            send_message_to_alive_players(room_id, MESSAGES['message_to_players'].format(sender_name=room['players'].get(user_id, "Неизвестный"), text=message.text),
                                          exclude_player_id=user_id, keyboard_func=lambda rid, pid, timer_val: get_player_game_keyboard(rid, pid, timer_val))
            logger.info(f"User {room['players'].get(user_id, 'Unknown')} sent chat message in day phase in room {room_id}: '{message.text}'")

    else: # If user not in any game room
        if user_states.get(message.from_user.id) not in ['awaiting_max_players_count', 'awaiting_room_id', 'in_room']:
            try:
